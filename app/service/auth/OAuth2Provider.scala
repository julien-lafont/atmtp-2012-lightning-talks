...
trait OAuth2Provider extends GenericProvider {
  ...
  override def auth(redirectRoute: Call)(implicit request: RequestHeader): Result = {
    request.queryString.get("code").flatMap(_.headOption).map { code =>
      ...
      redirectToAccreditationPage // Step one : redirect the user to the service's accreditation page
    }
  }
  ...
  private def retrieveAccessToken(code: String, redirectRoute: Call)(implicit request: RequestHeader) = {
    ...
    val verif_csrf = request.queryString.get("state").flatMap(_.headOption)
        ...
        fetchAccessTokenResponse(code).map(response =>
          processToken(response) match {
            ...
            case Token(Some(token), Some(expires)) =>
              Redirect(redirectRoute).withSession(request.session + (fieldToken -> token) + (fieldExpires -> expires.toString) + ("provider" -> name))

            // Provider return only token
            case Token(Some(token), None) =>
              Redirect(redirectRoute).withSession(request.session + (fieldToken -> token) + ("provider" -> name))
