...
case class Session(
	...
	def ajouterVote() = this.copy(vote = vote+1)
	def ajouterConsultation() = this.copy(consultation = consultation + 1)
	def withDetails(implicit request: RequestHeader) = Talk(this, Orateur.findById(orateurId).get, Vote.peutVoter(this)) 
}
...
object Session extends ModelCompanion[Session, ObjectId] { 
  ...
  def voter(session: Session)(implicit req : RequestHeader) : Session = { 
  	val sessionVotee = session.ajouterVote()
  	dao.save(sessionVotee)
